...
public class FirmaElectronicaService {
    ...
    public FirmaElectronica buscarFirma(FirmaElectronica data) {
        FirmaElectronica fd = repository.findFirstByUsuario_ReferenceIdOrderByFechaCreacionDesc(data.getUsuario().getReferenceId());
    ...
    public FirmaElectronica guardarFirma(FirmaElectronica data) {
        System.out.println("Guardar Firma " + data.toString());
        data.setFechaCreacion(Utils.getFechaMongo());
