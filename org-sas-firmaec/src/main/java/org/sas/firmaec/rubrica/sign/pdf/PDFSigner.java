 ...
 */
package org.sas.firmaec.rubrica.sign.pdf;
...
import org.sas.firmaec.rubrica.exceptions.InvalidFormatException;
import org.sas.firmaec.rubrica.exceptions.RubricaException;

import org.sas.firmaec.rubrica.sign.SignInfo;
import org.sas.firmaec.rubrica.sign.Signer;
...
public class PDFSigner implements Signer {
    ...
    public byte[] sign(byte[] data, String algorithm, PrivateKey key, Certificate[] certChain, Properties xParams)
        ...
        PdfStamper stp;
        try {
            stp = PdfStamper.createSignature(pdfReader, baos, '\u0000', (File) null, true);
        } catch (DocumentException var42) {
            logger.severe("Error al crear la firma para estampar: " + var42);
            throw new RubricaException("Error al crear la firma para estampar", var42);
        }
            ...
            try {
                ...
                switch (typeSig) {
                    case "QR": {
                        ...
                        Font font1 = new Font(0, fontSize + fontSize / 2.0F, 1, Color.BLACK);
                        float x = this.getMaxFontSize(BaseFont.createFont(), nombreFirmante.trim(), width - (width / 3.0F + 3.0F));
                        font1.setSize(x);
                        Paragraph paragraph = new Paragraph("Firmado electrónicamente por:\n", new Font(0, fontSize / 1.25F, 0, Color.BLACK));
                        paragraph.add(new Paragraph(nombreFirmante.trim(), font1));
                        paragraph.setAlignment(0);
                        paragraph.setLeading(x);
                        ...
                        columnTextImage.addElement(Image.getInstance(bufferedImage, (Color) null));
                        columnTextImage.go();
                    ...
                    /*    PdfTemplate pdfTemplate1 = sap.getLayer(2);
                        ...
                        text = text + infoQR;
                        try {
                            bufferedImage = org.bcbg.rubrica.utils.QRCode.generateQR(text, (int) height, (int) height);
                        } catch (Exception e) {
                            e.printStackTrace();
                        }
                        // QR
        ...
        try {
            ...
            if (stp != null) {
                try {
                    stp.close();
                } catch (Exception e) { /* log si quieres */ }
            }
            if (pdfReader != null) {
                try {
                    pdfReader.close();
