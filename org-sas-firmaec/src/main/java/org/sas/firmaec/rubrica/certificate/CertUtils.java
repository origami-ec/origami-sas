...
import org.sas.firmaec.rubrica.keystore.KeyStoreUtilities;
...
public class CertUtils {
    ...
    public static String    seleccionarAlias(KeyStore keyStore) {
        ...
        try {
            ...
            if (signingAliases.size() == 1) {
                ...
                for (Alias a : signingAliases) {
                    System.out.println("Tokens: " + a.getName() + " " + a.getAlias());
                    System.out.println();
                }
                aliasString = signingAliases.get(0).getAlias();
            }
    ...
    public static List<Alias> getTokens(KeyStore keyStore) throws RubricaException {
        List<Alias> signingAliases = KeyStoreUtilities.getSigningAliases(keyStore);
        if (signingAliases.isEmpty()) {
            return new ArrayList<>();
        }
        return signingAliases;
    }
